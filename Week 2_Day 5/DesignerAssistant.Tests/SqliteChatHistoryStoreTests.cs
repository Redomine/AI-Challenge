...
public sealed class SqliteChatHistoryStoreTests
{
    ...
    public async Task HistorySurvivesStoreRecreationAndCanBeCleared()
    {
        ...
        try
        {
            ...
            var contextState = ContextState.CreateDefault() with
            {
                Strategy = ContextStrategy.StickyFacts,
                Facts = new Dictionary<string, string> { ["расход"] = "4800 м³/ч" }
            };
            await reopenedStore.SaveContextStateAsync(contextState);

            var thirdStore = new SqliteChatHistoryStore(databasePath);
            await thirdStore.InitializeAsync();
            var restoredState = await thirdStore.LoadContextStateAsync();
            Assert.Equal(ContextStrategy.StickyFacts, restoredState.Strategy);
            Assert.Equal("4800 м³/ч", restoredState.Facts["расход"]);

            await reopenedStore.ClearAsync();
            Assert.Empty(await reopenedStore.LoadAsync());
            Assert.Equal(
                ContextStrategy.SlidingWindow,
                (await reopenedStore.LoadContextStateAsync()).Strategy);
        }
        finally
        {
