    ...
    int SummaryBatchSize)
{
    public static AppOptions FromEnvironment()
    {
        ...
        var recentMessageCount = ReadPositiveInt(
            "DESIGN_ASSISTANT_RECENT_MESSAGES",
            10);
        var summaryBatchSize = ReadPositiveInt(
            "DESIGN_ASSISTANT_SUMMARY_BATCH_SIZE",
            10);
    ...
    private static int ReadPositiveInt(string variableName, int defaultValue)
    {
        ...
        var result = int.TryParse(value, out var parsed) ? parsed : defaultValue;
        if (result <= 0)
        {
            throw new InvalidOperationException(
                $"{variableName} должно быть положительным числом.");
        }
